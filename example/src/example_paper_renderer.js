      ...
      dom(
        ...
        ...limiters.map((limiter) =>
          dom(
            "span",
            ["badge", "warning"],
            (el) => (el.innerText = stringify(limiter)),
          ),
        ),
        dom("input", null, (el) => {
          el.type = "text";
  ...
  for (const modifier of modifiers) {
    domNode.appendChild(
      dom(
        "label",
        null,
        (el) => (el.innerText = modifier.property),
        dom("input", null, (el) => {
  ...
  for (const feature of features) {
    domNode.appendChild(
      dom(
        ...
        dom("div", ["definitions"], (el) =>
          renderDefinitions(el, feature.definitions),
        ),
        dom("div", ["modifiers"], (el) =>
          renderModifiers(el, feature.modifiers),
        ),
      ),
    );
  }
    ...
    domNode.appendChild(
      dom(
        ...
        dom(
          "div",
          ["card-body"],
          (el) => renderFeatures(el, featureSet.features),
          dom(
            "p",
            ["card-text"],
            (el) => (el.innerText = featureSet.description),
          ),
        ),
      ),
    );
